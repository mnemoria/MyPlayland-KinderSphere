...
if (!empty($searchInput)) {
    ...
    $whereClause .= " WHERE firstname LIKE '%$searchInput%' or lastname LIKE '%$searchInput%'";
}
...
if (!empty($filterStatus) && $filterStatus !== 'All') {
    ...
    $whereClause .= "employment_status = '$filterStatus'";
}

// Fetch data from the database
$query = "SELECT * FROM teacher_info" . $whereClause . " LIMIT ?, ?";
...
while ($row = mysqli_fetch_assoc($result)) {
    // Fetch the image data as base64 encoded string
...
// "All" filter
if ($filterStatus === 'All') {
    $allQuery = "SELECT COUNT(*) AS total FROM teacher_info";
    ...
    $allQuery = "SELECT * FROM teacher_info LIMIT ?, ?";
    ...
    $totalRecordsQuery = "SELECT COUNT(*) AS total FROM teacher_info" . $whereClause;
