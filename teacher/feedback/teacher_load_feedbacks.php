...
//   student_info s
// INNER JOIN
//   feedback_table f ON s.id = f.student_id AND s.class_id = f.class_id;
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
          ...
          echo '  <div class="card-header">Student Name: ' . $firstname . ' ' . $lastname . '</div>';
          echo '  <div class="card-body">';
          echo '    <p class="card-text">Email: ' . $email . '</p>';
          echo '    <p class="card-text">Feedback: ' . $feedback_text . '</p>';
          echo '    <p class="card-text"><small class="text-muted">Date Added: ' . $date_added . '</small></p>';
          echo '  </div>';
          echo '</div>';
        }
      } else {
